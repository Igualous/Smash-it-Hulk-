...
GAME_LOOP:  
	...
	CHITAURI_PRINT:
         # Função para restaurar o fundo na região ocupada pelo Chitauri
# Argumentos:
# a0: endereço do fundo
# a1: posição X do Chitauri
# a2: posição Y do Chitauri

RESTAURA_FUNDO_CHITAURI:
   la t0,CHITAURI_POS
   lw a1,0(t0)
   lw a2,4(t0)
    # Carregar endereço do fundo
    la t0, fundo1  # Endereço do fundo
    li t1, 0xFF000000  # Endereço inicial da Memória VGA - Frame 0

    # Calcular o endereço inicial no fundo
    li t2, 320  # Largura do fundo
    mul t3, a2, t2  # Y * largura do fundo
    add t3, t3, a1  # Y * largura + X
    add t0, t0, t3  # Endereço inicial no fundo

    # Calcular o endereço inicial na VGA
    mul t3, a2, t2  # Y * largura do fundo
    add t3, t3, a1  # Y * largura + X
    add t1, t1, t3  # Endereço inicial na VGA

    # Loop para copiar a região do fundo para a VGA
...
RESTAURA_COLUNA:
    beqz t5, PROXIMA_LINHA  # Se terminou todas as colunas, ir para a próxima linha

    lb t3, 0(t0)  # Carrega o pixel do fundo
    sb t3, 0(t1)  # Escreve o pixel na VGA

    addi t0, t0, 1  # Próximo pixel no fundo
    addi t1, t1, 1  # Próximo pixel na VGA
...
PROXIMA_LINHA:
    ...
    addi t0, t0, 286  # Avança para a próxima linha no fundo (320 - 34)
    addi t1, t1, 286  # Avança para a próxima linha na VGA (320 - 34)
